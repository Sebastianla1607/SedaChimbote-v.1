...
const register = async (req, res) => {
  try {
    ...
    if (!customer) {
      return res
        .status(404)
        .json({ error: "Número de suministro no encontrado" });
    }

    // 2. Validar referencia de cobro
    if (
      parseFloat(reference_amount) !== parseFloat(customer.reference_amount)
    ) {
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          error: "Los datos personales no coinciden con los registrados",
        });
    }
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ error: "Este suministro ya tiene una cuenta registrada" });
    }
