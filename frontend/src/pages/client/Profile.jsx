...
import { User, Lock, Eye, EyeOff, LogOut, CheckCircle, Mail, Home } from 'lucide-react'
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 relative overflow-hidden">
      ...
      <div className="bg-slate-900/40 border-b border-slate-900/60 px-4 pt-10 pb-8 flex flex-col items-center">
        ...
        <span className="bg-blue-600/20 text-blue-400 border border-blue-500/20 text-[10px] uppercase font-bold tracking-wider px-3 py-1 rounded-full mt-1.5">
      ...
      <div className="flex-grow px-4 py-6 space-y-4 pb-24 md:px-8 md:grid md:grid-cols-2 md:gap-6 md:space-y-0 md:items-start max-w-4xl w-full mx-auto">
        ...
        {success && <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-2xl px-4 py-3 text-sm font-semibold flex items-center gap-2 md:col-span-2 mb-4"><CheckCircle className="w-4 h-4 text-emerald-400" />{success}</div>}

        <div className="card overflow-hidden p-0">
          <div className="px-4 py-3 border-b border-slate-800/80">
            <p className="label mb-0">Datos Personales</p>
          </div>
        ...
        <button onClick={logout} className="w-full bg-slate-900/60 border border-rose-500/20 text-rose-500 font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-rose-500/10 transition active:scale-[0.98]">
