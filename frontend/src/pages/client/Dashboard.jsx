...
import { Bell, Plus, Home, Ticket, Map, User, Clock, AlertTriangle, CheckCircle, Loader, XCircle, Eye } from 'lucide-react'
...
const statusConfig = {
  PENDIENTE: { label: 'Pendiente', color: 'bg-gray-100 text-gray-600', icon: <Clock className="w-3 h-3" /> },
  ASIGNADO: { label: 'Asignado', color: 'bg-blue-100 text-blue-700', icon: <Loader className="w-3 h-3" /> },
  EN_CAMINO: { label: 'En camino', color: 'bg-indigo-100 text-indigo-700', icon: <Loader className="w-3 h-3" /> },
  EJECUCION_ACTIVA: { label: 'En proceso', color: 'bg-purple-100 text-purple-700', icon: <Loader className="w-3 h-3" /> },
  PRE_CERRADO: { label: 'Pre-cerrado', color: 'bg-teal-100 text-teal-700', icon: <CheckCircle className="w-3 h-3" /> },
  OBSERVADO: { label: 'Observado', color: 'bg-orange-100 text-orange-700', icon: <AlertTriangle className="w-3 h-3" /> },
  CERRADO: { label: 'Cerrado', color: 'bg-green-100 text-green-700', icon: <CheckCircle className="w-3 h-3" /> },
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col max-w-md mx-auto relative">
      ...
      <div className="bg-[#1a237e] px-4 pt-10 pb-6">
        ...
        <h2 className="text-white text-xl font-bold">Hola, {user?.first_name} 👋</h2>
      ...
      <div className="bg-white border-b border-gray-200 px-4">
        <div className="flex">
          ...
          >
            Activos {activeTickets.length > 0 && <span className="ml-1 bg-[#1a237e] text-white text-xs px-1.5 py-0.5 rounded-full">{activeTickets.length}</span>}
          </button>
          <button
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 font-medium">
              {tab === 'activos' ? 'No tienes reclamos activos' : 'Sin historial de reclamos'}
            </p>
            <p className="text-gray-400 text-sm mt-1">
              {tab === 'activos' ? 'Presiona el botón para reportar uno' : 'Aquí aparecerán tus reclamos cerrados'}
            </p>
          </div>
        ) : (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center gap-1 text-xs text-gray-400">
                  ...
                  Hace {ticket.days_elapsed} día{ticket.days_elapsed !== 1 ? 's' : ''}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1 ${priorityConfig[ticket.priority]?.color}`}>
                  {priorityConfig[ticket.priority]?.label}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1 ${statusConfig[ticket.status]?.color}`}>
              ...
              {ticket.assigned_esp?.access_code && (
                <div className="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">Técnico asignado: <span className="font-semibold text-gray-700">{ticket.assigned_esp.access_code}</span></span>
                </div>
              )}
            </div>
          ))
          ...
          <div className="bg-white rounded-t-2xl w-full max-h-[80vh] overflow-y-auto p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1 ${statusConfig[selectedTicket.status]?.color}`}>
                  {statusConfig[selectedTicket.status]?.icon}
                  {statusConfig[selectedTicket.status]?.label}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${priorityConfig[selectedTicket.priority]?.color}`}>
              ...
              {selectedTicket.ai_category && (
                <div className="bg-blue-50 rounded-lg p-3">
                  <p className="text-xs text-blue-600 font-semibold mb-1">ANÁLISIS DE IA</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs text-gray-500 font-semibold mb-1">CREADO</p>
                  <p className="text-sm text-gray-700">{new Date(selectedTicket.created_at).toLocaleDateString('es-PE')}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs text-gray-500 font-semibold mb-1">VENCE</p>
